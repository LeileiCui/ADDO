...
##' ADDO_Heterotic1_QC(indir=indir, outdir=outdir, Input_name="TEST", Kinship_type="GCTA_ad", PheList=c("sex","batch"), covariates_sum=2)
##'
##' @author Leilei Cui and Bin Yang

ADDO_Heterotic1_QC <- function(indir = indir,
                               outdir = outdir,
...
if(Input_type == "PLINK"){
	...
	Phe_Raw = read.header(paste(indir,"/",Input_name,".phe",sep=""))
	rownames(Phe_Raw) = Phe_Raw[,1]; colnames(Phe_Raw)[1] = "id"
}
...
if(Input_type == "GenABEL"){
	...
	if(!file.exists("Data_raw.tped")){system("plink --noweb --silent --file Data_raw --transpose --recode --out Data_raw")}
