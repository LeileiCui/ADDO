...
Phe_Clean = read.table(paste(rawdir,"/../2_PheClean/Phe_Clean.txt",sep=""),header=T)
	...
	if(!file.exists(paste(outdir,'/2_Pvalue/Pvalue_',phenotype_name,'.txt.tar.gz',sep=''))){
		...
		try({
			...
			if(num_nodes == 1){
				...
				interval_length = as.integer(dim(Chip_ped)[2]/num_nodes); if(interval_length %% 2 != 0){interval_length = interval_length+1}
				...
				if(i == num_nodes){ analysis_allele = (7+(i-1)*interval_length):dim(Chip_ped)[2]; analysis_genotype = ((i-1)*interval_length/2+1):((dim(Chip_ped)[2]-6)/2) }
			}
...
Plot_Phe_nor = mclapply((covariates_sum+2):length(PheList),Plot_Phe_Histogram,Phe_data=Phe_Trans,Phe_names=PheList,mc.cores=num_nodes)
}
