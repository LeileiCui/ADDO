...
try({
	...
	if(!file.exists(paste(outdir,'/0_All/All_',phenotype_name,'.png',sep=''))){
		...
		try({
			...
			TEMP.ld <- read.table(file="RegionMan_tmp.ld",header=T,stringsAsFactors=F); rownames(TEMP.ld) = TEMP.ld[,"SNP_B"]
		...
		try({
			...
			TEMP.geno <- read.table(file="Boxplot_tmp.ped",header=F,stringsAsFactors=F); system("rm Boxplot_tmp.*")
